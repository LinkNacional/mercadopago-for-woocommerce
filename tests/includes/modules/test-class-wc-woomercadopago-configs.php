...
class WC_WooMercadoPago_ConfigsTest extends WP_UnitTestCase {
    public function setUp(): void {
        require_once dirname( __DIR__ ) . '/../../includes/module/class-wc-woomercadopago-configs.php';
        require_once dirname( __DIR__ ) . '/../../includes/module/class-wc-woomercadopago-credentials.php';
        require_once dirname( __DIR__ ) . '/../../includes/module/class-wc-woomercadopago-module.php';
        require_once dirname( __DIR__ ) . '/../../includes/module/class-wc-woomercadopago-options.php';
        require_once dirname( __DIR__ ) . '/../../includes/helpers/class-wc-woomercadopago-helper-credits.php';
     ...
     * Get Country Configs.
     */
    public function test_get_country_configs(): void {
     ...
     */
    public function test_set_payment_gateway(): void {
        $credits_helper_mock = $this->getMockBuilder('WC_WooMercadoPago_Helper_Credits')
            ->setMethods(array('is_credits'))
            ->getMock();
        $credits_helper_mock->expects($this->any())
            ->method('is_credits')
            ->willReturn(1);
			
        $woomercadoPago_configs = $this->getMockBuilder('WC_WooMercadoPago_Configs')
            ->setMethods(array('get_credits_helper_instance'))
            ->getMock();
        $woomercadoPago_configs->expects($this->any())
            ->method('get_credits_helper_instance')
            ->willReturn($credits_helper_mock);

        $payment_gateway = $woomercadoPago_configs->set_payment_gateway( array() );
    ...
    public function test_get_available_payment_methods(): void {
        ...
        $payment_gateway = $woomercadoPago_configs->get_available_payment_methods( array() );
        $methods_returned = array (
            'WC_WooMercadoPago_Basic_Gateway',
            'WC_WooMercadoPago_Custom_Gateway',
            'WC_WooMercadoPago_Ticket_Gateway',
