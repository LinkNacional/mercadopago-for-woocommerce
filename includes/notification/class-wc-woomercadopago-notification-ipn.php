...
class WC_WooMercadoPago_Notification_IPN extends WC_WooMercadoPago_Notification_Abstract {
    ...
    public function check_ipn_response(): void {
        ...
        if ( ! isset( $data['id'] ) || ! isset( $data['topic'] ) ) {
            ...
            $this->set_response( 422, null, __( 'No ID or TOPIC param in Request IPN', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
        }

        if ( 'payment' === $data['topic'] || 'merchant_order' !== $data['topic'] ) {
            $this->set_response( 200, null, __( 'Discarded notification. This notification is already processed as webhook-payment.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
        }
        ...
        if ( 'merchant_order' === $data['topic'] ) {
            ...
            if ( is_wp_error( $ipn_info ) || ( 200 !== $ipn_info['status'] && 201 !== $ipn_info['status'] ) ) {
                ...
                $this->set_response( 422, null, __( 'IPN merchant_order not found', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
            }

            $payments = $ipn_info['response']['payments'];
            if ( count( $payments ) < 1 ) {
                $this->log->write_log( __FUNCTION__, 'Not found Payments into Merchant_Order' );
                $this->set_response( 422, null, __( 'Not found Payments into Merchant_Order', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
            }
