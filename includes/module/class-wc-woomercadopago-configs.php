...
class WC_WooMercadoPago_Configs {
     ...
     */
    private function update_token_new_version(): void {
        if ( empty( get_option( '_mp_public_key_prod', '' ) ) || empty( get_option( '_mp_access_token_prod', '' ) ) ) {
     ...
     */
    private function update_payments(): void {
     ...
     */
    private function update_ticket_methods(): void {
        $mp_instance = WC_WooMercadoPago_Module::get_mp_instance_singleton();
        if ( $mp_instance ) {
     ...
     * @throws WC_WooMercadoPago_Exception Update ticket exception.
     */
    private function update_pix_methods(): void {
        $mp_instance = WC_WooMercadoPago_Module::get_mp_instance_singleton();
        if ( $mp_instance ) {
    ...
    public function notice_update_access_token(): void {
        $type = 'error';
        $message = __( 'Update your credentials with the Access Token and Public Key, you need them to continue receiving payments!', LKN_WC_MERCADOPAGO_TEXT_DOMAIN);
    ...
    public function notice_https(): void {
        $type = 'notice-warning';
        $message = __( 'The store should have HTTPS in order to activate both Checkout Personalizado and Ticket Checkout.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN);
        echo wp_kses_post(WC_WooMercadoPago_Notices::get_alert_frame($message, $type ));
    }
