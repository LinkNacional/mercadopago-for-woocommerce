...
class MP {
     ...
     */
    public function set_locale( $country_code ): void {
     ...
     * Set payment class
     *
     * @param WC_WooMercadoPago_Payment_Abstract|null $payment Payment class.
     */
    public function set_payment_class( $payment = null ): void {
        if ( ! is_null( $payment ) ) {
     ...
     * @param int $ttl
     */
    protected function set_cache_response( $key, $value, $ttl = MINUTE_IN_SECONDS ): void {
        $key = sha1( $key );
     ...
     * @param $message
     */
    protected function debug_mode_log( $log_id, $function, $message ): void {
     ...
     * @param $lpayment_id
     */
    private function build_payment_places( &$api_response ): void {
        $payment_places =
        	array(
        	    'paycash' => array(
        	        array(
