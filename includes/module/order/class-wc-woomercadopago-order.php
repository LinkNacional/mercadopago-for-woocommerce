...
class WC_WooMercadoPago_Order {
    ...
    public function mp_rule_approved( $data, $order, $used_gateway ): void {
        ...
        if ( 'pending' === $status || 'on-hold' === $status || 'failed' === $status ) {
            $order->add_order_note( 'Mercado Pago: ' . __( 'Payment approved.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
            ...
            switch ( $used_gateway ) {
                case 'WC_WooMercadoPago_Pix_Gateway':
                    ...
                    $order->add_order_note(
                        'Mercado Pago: ' . __( 'Waiting for the Pix payment.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
                    );
                ...
                case 'WC_WooMercadoPago_Ticket_Gateway':
                    ...
                    $order->add_order_note(
                        'Mercado Pago: ' . __( 'Waiting for the ticket payment.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
                    );
                ...
                default:
                    $order->add_order_note(
                        'Mercado Pago: ' . __( 'The customer has not made the payment yet.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
                    );
                    break;
            }
        } else {
    ...
    public function mp_rule_in_process( $data, $order ): void {
        if ( $this->can_update_order_status( $order ) ) {
            $order->update_status(
                ...
                'Mercado Pago: ' . __( 'Payment is pending review.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
            );
        } else {
    ...
    public function mp_rule_rejected( $data, $order ): void {
        if ( $this->can_update_order_status( $order ) ) {
            $order->update_status(
                ...
                'Mercado Pago: ' . __( 'Payment was declined. The customer can try again.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
            );
        } else {
    ...
    public function mp_rule_refunded( $order ): void {
        $order->update_status(
            ...
            'Mercado Pago: ' . __( 'Payment was returned to the customer.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
        );
    }
    ...
    public function mp_rule_cancelled( $data, $order ): void {
        if ( $this->can_update_order_status( $order ) ) {
            $order->update_status(
                ...
                'Mercado Pago: ' . __( 'Payment was canceled.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
            );
        } else {
    ...
    public function mp_rule_in_mediation( $order ): void {
        ...
        $order->add_order_note(
            'Mercado Pago: ' . __( 'The payment is in mediation or the purchase was unknown by the customer.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
        );
    }
    ...
    protected function validate_order_note_type( $data, $order, $status ): void {
        ...
        $order->add_order_note(
            sprintf(
                ...
                __( 'Mercado Pago: The payment %1$s was notified by Mercado Pago with status %2$s.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                $payment_id,
                $status
            )
        );
