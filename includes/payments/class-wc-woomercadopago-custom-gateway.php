...
class WC_WooMercadoPago_Custom_Gateway extends WC_WooMercadoPago_Payment_Abstract {
     ...
     * @param $order_id
     */
    public function display_installment_fee_order( $order_id ): void {
        $order = wc_get_order( $order_id );
        $order->get_meta_data();
     ...
     * @param array $params
     */
    public function display_order_note( $params ): void {
        wc_get_template(
     ...
     * @param $order
     */
    protected function process_discount_and_commission( $order_id, $order ): void {
