...
class WC_WooMercadoPago_Pix_Gateway extends WC_WooMercadoPago_Payment_Abstract {
    ...
    public function __construct() {
        $this->id = self::ID;
        $this->title = __( 'Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN );
        $this->title_gateway = __( 'Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN );
        ...
        $this->title = $this->get_option( 'title', __( 'Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
     ...
     * @return void
     */
    public function update_pix_method(): void {
    ...
    public function field_enabled() {
        return array(
            ...
            'subtitle' => __( 'By disabling it, you will disable all Pix payments from Mercado Pago Transparent Checkout.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            'type' => 'mp_toggle_switch',
            'default' => 'no',
            'descriptions' => array(
                'enabled' => __( 'The transparent checkout for Pix payment is <b>enabled</b>.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'disabled' => __( 'The transparent checkout for Pix payment is <b>disabled</b>.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            ),
        );
    }
    ...
    public function field_checkout_steps_pix() {
        $steps_content = wc_get_template_html(
            ...
            array(
                'title' => __( 'To activate Pix, you must have a key registered in Mercado Pago.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_one_text' => __( 'Download the Mercado Pago app on your cell phone.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_two_text_one' => __( 'Go to the ', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_two_text_two' => __( 'area and choose the ', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_two_text_highlight_one' => __( 'Your Profile ', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_two_text_highlight_two' => __( 'Your Pix Keys section.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'step_three_text' => __( 'Choose which data to register as Pix keys. After registering, you can set up Pix in your checkout.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'observation_one' => __( 'Remember that, for the time being, the Central Bank of Brazil is open Monday through Friday, from 9am to 6pm.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'observation_two' => __( 'If you requested your registration outside these hours, we will confirm it within the next business day.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'button_about_pix' => __( 'Learn more about Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'observation_three' => __( 'If you have already registered a Pix key at Mercado Pago and cannot activate Pix in the checkout, ', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'link_title_one' => __( 'click here.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
    ...
    public function field_checkout_pix_header() {
        return array(
            'title' => sprintf(
                ...
                ' . __( 'Transparent Checkout | Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) . '
                 </div>
    ...
    public function field_checkout_pix_payments_advanced_title() {
        return array(
            'title' => __( 'Advanced configuration of the Pix experience', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            'type' => 'title',
    ...
    public function field_pix_date_expiration() {
        $pix_expiration_values = array(
            '15 minutes' => __( '15 minutes', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '30 minutes' => __( '30 minutes (recommended)', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '60 minutes' => __( '60 minutes', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '12 hours' => __( '12 hours', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '24 hours' => __( '24 hours', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '2 days' => __( '2 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '3 days' => __( '3 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '4 days' => __( '4 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '5 days' => __( '5 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '6 days' => __( '6 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            '7 days' => __( '7 days', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
        );

        return array(
            'title' => __( 'Expiration for payments via Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            'type' => 'select',
            'description' => __( 'Set the limit in minutes for your clients to pay via Pix.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            'default' => '30 minutes',
    ...
    public function field_checkout_about_pix() {
        $link_content = wc_get_template_html(
            ...
            array(
                'title' => __( 'Want to learn how Pix works?', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'subtitle' => __( 'We have created a page to explain how this new payment method works and its advantages.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'url_link' => 'https://www.mercadopago.com.br/pix/',
                'button_text' => __( 'Learn more about Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
            ),
    ...
    public function field_checkout_pix_card_info() {
        $value = array(
            'title' => __('Would you like to know how Pix works?', LKN_WC_MERCADOPAGO_TEXT_DOMAIN), __('Important! To sell you must enter your credentials.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN),
            'subtitle' => __('We have a dedicated page where we explain how it works and its advantages.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN),
            'button_text' => __('Find out more about Pix', LKN_WC_MERCADOPAGO_TEXT_DOMAIN),
            ...
            if ( is_array( $response ) && array_key_exists( 'status', $response ) ) {
                ...
                if ( 'pending' === $response['status'] ) {
                    if ( 'pending_waiting_payment' === $response['status_detail'] || 'pending_waiting_transfer' === $response['status_detail'] ) {
                        ...
                        $order->update_meta_data( 'checkout_pix_date_expiration', __( $this->get_option( 'checkout_pix_date_expiration', '30 minutes' ), LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) );
                        ...
                        $order->add_order_note(
                            'Mercado Pago: ' .
                            __( 'The customer has not paid yet.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN )
                        );

                        if ( 'pix' === $response['payment_method_id'] ) {
                            $order->add_order_note(
                                '<div style="text-align: justify;"><p>Mercado Pago: ' . __( 'Now you just need to pay with Pix to finalize your purchase.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) . ' ' .
                                __( 'Scan the QR code below or copy and paste the code into your bank\'s application.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ) . '</small></p>',
                                1,
                                false
                            );
                        }
     ...
     * @return void
     */
    public function enable_pix_payment_notice(): void {
        $type = 'notice-warning';
        $message = wc_get_template_html(
            'checkout/credential/alert/alert-pix-not-registered.php',
            array(
                'message' => __( 'Please note that to receive payments via Pix at our checkout, you must have a Pix key registered in your Mercado Pago account.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
                'text_link' => __( 'Register your Pix key at Mercado Pago.', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
    ...
    public static function get_pix_template( $order ) {
        $pix_on = (array) $order->get_meta( 'pix_on' );
        $pix_on = (int) array_pop( $pix_on );

        if ( 1 === $pix_on && 'pending' === $order->get_status() ) {
            $mp_pix_qr_code = $order->get_meta( 'mp_pix_qr_code' );
            ...
            $pix_template = wc_get_template(
                ...
                array(
                    ...
                    'text_expiration_date' => __( 'Code valid for ', LKN_WC_MERCADOPAGO_TEXT_DOMAIN ),
